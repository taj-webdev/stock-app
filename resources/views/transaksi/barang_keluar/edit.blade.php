...
<div class="animate-fadeIn space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <form action="{{ route('barang-keluar.update', $data->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="date" name="tanggal" value="{{ $data->tanggal }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-orange-500 focus:border-orange-500 px-4 py-2"
                       required>
            </div>
            ...
            <div>
                ...
                <input type="text" name="keterangan" value="{{ $data->keterangan }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-orange-500 focus:border-orange-500 px-4 py-2"
                       placeholder="Opsional, misal: retur barang, permintaan pelanggan">
            </div>

            {{-- Tombol --}}
            <div class="flex items-center gap-3 pt-2">
                <button type="submit"
                        class="inline-flex items-center gap-2 px-5 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-all">
