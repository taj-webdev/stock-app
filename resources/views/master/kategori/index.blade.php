...
<div class="animate-fadeIn space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        <h2 class="text-2xl font-semibold text-gray-800 flex items-center gap-2">
            <i data-lucide="tags" class="w-6 h-6 text-blue-500"></i>
            Master Kategori
        </h2>
    ...
    <div class="bg-white rounded-xl shadow-md p-5">
        <form action="{{ route('kategori.store') }}" method="POST" class="flex flex-col md:flex-row gap-4 items-center">
            @csrf
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden">
        ...
        <div class="p-4 border-t flex flex-col sm:flex-row justify-between items-center gap-2">
            ...
            <div>
                {{ $kategori->appends(['search' => request('search')])->links('pagination::tailwind') }}
            </div>
        </div>
        @endif
    </div>
