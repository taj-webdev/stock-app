    ...
    <form method="GET" action="{{ route('laporan.barang-masuk') }}"
        ...
        <div>
            ...
            <input type="text" name="search" value="{{ $search }}"
                   placeholder="Cari barang / user / keterangan..."
                   class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2">
        </div>
        <div class="flex items-end gap-2">
            <button class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all">
                <i data-lucide="search" class="w-5 h-5"></i> Tampilkan
            </button>
            <a href="{{ route('laporan.barang-masuk.pdf', ['from' => $from, 'to' => $to]) }}"
               target="_blank"
